# FEATURES: Educational Simple Coder Agent (Planned: ReAct Pattern with TODO System)

Scope
- Educational use; **planned** multi-step execution with ReAct pattern; teaching focus on task decomposition and reasoning transparency; session context retained client-side; operations constrained to repository root; minimal UI (TODO panel planned, not yet implemented).
...
- Planned: ReAct loop in AgentService, TODO management (@Tool methods), bash/powershell tools, UI TODO panel.
...
- Note: Backend currently processes requests as single-turn; ReAct loop implementation pending. Tool selection & parameter extraction entirely driven by Spring AI tool-calling. UI dropdown is advisory only.
- Not implemented: ReAct loop, TODO system, bash/powershell tools, ToolsService unit tests, end-to-end integration tests (Controller + ChatClient + real model).

Feature List (Target Capabilities)
- **ReAct Loop (Planned)**: Multi-step execution with Reasoning → Acting → Observing cycle (max 10 steps).
...
- **Step tracking**: Each loop iteration increments step counter; exposed to agent via system message.
...
TODO / Diff Tracking
- [ ] ReAct loop implementation in AgentService (while loop with step counter)
...
- [ ] Step limit enforcement (max 10 steps)
